/*global console, $, Survey, addDirectionClassToBody */

// .prop() is available in jQuery 1.6+; otherwise, fall back to .attr()
$.fn.prop = $.fn.prop || $.fn.attr;

jQuery(document).ready(function () {
  var gridsAutoOptimize = $(),
    gridsForceDesktop = $(),
    gridsGroupByColCount = 0,
    rowClasses = [],
    lastWindowWidth,
    gridWidths = [],
    gridParentWidths = [],
  ...
  function handleOEKeypress() {
    ...
    var $oe = $(this);
    if ($oe.hasClass("mobileOE")) {
      // Skip if OE isn't approved for jumping.
      $oe
        .closest(".grid")
        .find('.mobile-oe-legend[data-for="' + $oe.attr("id") + '"]')
        .html($oe.val());
    }
    return true;
  }
  ...
  function moveOE($cell, speed) {
    /* Method for moving an OE to a target cell */

    // If an accordion, only move the OE if it's inside an open row
    var currRowIsOpen =
      $cell.parents(".row.row-elements.open-accordion").length > 0;
    var $openAccordions = $cell
    ...
    if ($openAccordions.length !== 0 && !currRowIsOpen) {
      return;
    }

    var $span = $cell.find(".mobile-oe-legend"),
      $oe = $("#" + $span.attr("data-for").replace(/\./g, "\\."));
    ...
    $oe.hide(0, function () {
      ...
      $oe.appendTo($cell.find(".cell-text")).show(speed);
  ...
  function optimizeOE(telement, isMobile) {
    ...
    if (
      gridSetting(telement, "single-col") ||
      gridSetting(telement, "group-by-col")
    ) {
      ...
      if (isMobile || telement.hasClass("grid-list-mode")) {
        ...
        telement
          ...
          .each(function () {
            ...
            var $oe = $(this).addClass("mobileOE");
            // Scenario single-column
            if (gridSetting(telement, "single-col")) {
              ...
              if ($oe.hasClass("oe-left")) {
                $oe.appendTo($oe.parent().next(".element").find(".cell-text"));
              } else {
                $oe.appendTo($oe.parent().prev(".element").find(".cell-text"));
              }
              // Scenario group-by-col multi-column
            } else {
              // Bind the keypress event if we haven't already.
              if (!$oe.hasClass("supportsMobileOE")) {
                $oe.addClass("supportsMobileOE").keyup(handleOEKeypress);
              }
              ...
              $oe
                ...
                .each(function () {
                  ...
                  if (!$cell.hasClass("supportsOE")) {
                    // Inject the span into the cell and bind its click event, if we haven't already. Otherwise, just update the text.
        ...
        telement.find(".mobileOE").each(function () {
          ...
          if ($oe.parent().hasClass("cell-text")) {
            // Unhide the span right next to the OE.
            $oe.parent().siblings(".mobile-oe-legend").show(0);
            // Move the OE to the appropriate cell.
            if ($oe.hasClass("oe-left")) {
              $oe.appendTo($oe.closest(".cell").prevAll(".row-legend")).show(0);
            } else {
              $oe.appendTo($oe.closest(".cell").nextAll(".row-legend")).show(0);
            }
          }
        });
      }
        ...
        $uniqueOEs.each(function () {
          ...
          $oe
            ...
            .each(function () {
              ...
              var $cell = $(this).find(".cell").eq(index);
              // Only add class to the first cell if it doesn't already have the input field
              if (!$cell.hasClass("activeOE")) {
                $cell.addClass("awaitingOE");
              }
        ...
        telement.find(".mobileOE").each(function () {
          ...
          var $oe = $(this).removeClass("mobileOE"),
            index = $oe
              .closest(".row")
              .find(".cell")
              .index($oe.closest(".cell"));
          // Loop through each row with element cells.
          telement.find(".row-elements").each(function () {
            // Remove the supporting classes from the row's element cells, so click events don't work.
            $(this).find(".cell").eq(index).removeClass("awaitingOE activeOE");
          });
          // Unhide the span right next to the OE.
  ...
  function regroupGroupByCols(telement, isMobile) {
    ...
    if (
      telement.hasClass("grid-multi-col") ||
      telement.hasClass("grid-multi-col-subgrid")
    ) {
      return;
    }
    ...
    telement.find(isMobile ? ".desktop" : ".mobile").each(function () {
      ...
      while (r > currowcount) {
        ...
        $(
          "<" +
            (simple ? "div" : "tr") +
            (height !== "None" ? ' style="height:' + height + ';"' : "") +
            ' id="-grid-row-' +
            t +
            "-" +
            currowcount +
            '">'
        ).insertAfter("#-grid-row-" + t + "-" + (currowcount - 1));
      }

      // Move the cell
      celement
        .toggleClass("mobile desktop")
        .appendTo("#-grid-row-" + t + "-" + r);
    });
    ...
    for (r = 1; r <= currowcount; r++) {
      ...
      if (isMobile) {
        ...
        $("#-grid-row-" + t + "-" + r).appendTo(
          "#-grid-tbody-" + t + "-" + rowTbodies[t - 1][r - 1]
        );
        document.getElementById("-grid-row-" + t + "-" + r).className =
          rowClasses[t - 1][r - 1];
      }
    }
  }
  ...
  function autosizeCols($telement, isMobile, autoOptimize, firstRun) {
    ...
    if (
      $telement.parent().parent().hasClass("noCols") ||
      $telement.parent().parent().hasClass("skipAutosize")
    ) {
      return true;
    }
    ...
    if (autoOptimize && !firstRun) {
      ...
      $cells.filter('[style*="width"]').each(function () {
        var $this = $(this),
          inlineWidth = $this.prop("style").width;
        if (inlineWidth) {
    ...
    if (!isMobile) {
      ...
      var max = 0;
      if (gridSetting($telement, "ss-colWidth")) {
        $telement.width("auto");
      }
      $cells.each(function () {
      ...
      if (autoOptimize) {
        ...
        if (!$telement.hasClass("setWidth")) {
          max = $cells.first().width();
        }
      } else {
        var $filler = $cells.find(".force-width");
        if ($filler.length === 0) {
          $filler = $cells.append(
            '<div class="force-width" style="width:' + max + 'px"></div>'
          );
        } else {
          $filler.width(max);
        }
      ...
      $cells.each(function () {
        if (Math.abs($(this).width() - max) > 1) {
          // Allow a 1-pixel margin for error.
          success = false;
          return false;
        }
      });
  ...
  function optimizeTables() {
    ...
    gridsAutoOptimize.each(function (i) {
      if (
        ...
        $(this).width() !== gridWidths[i] ||
        $(this).parent().parent().width() !== gridParentWidths[i]
      ) {
    ...
    gridsForceDesktop.each(function () {
      autosizeCols($(this), false, false);
    });
  }
  function optimizeTable(firstRun) {
    ...
    var $grid = $(this),
      wasMobile = $grid.hasClass("grid-list-mode");
    ...
    if (
      !autosizeCols($grid, false, true, firstRun) ||
      $grid.width() > $grid.parent().parent().width()
    ) {
      $grid
  ...
  function isScreenPainted() {
    ...
    return (
      gridsAutoOptimize.last().parent().parent().width() <= $(window).width()
    );
  }
  ...
  $(".grid").each(function (index) {
    ...
    if (
      gridSetting($grid, "auto-optimize") &&
      gridSetting($grid, "table-mode")
    ) {
      // Add to relevant lists and optimize immediately.
      if (gridsAutoOptimize.length > 0) {
        gridsAutoOptimize = gridsAutoOptimize.add($grid);
      } else {
        gridsAutoOptimize = $grid;
      }
      if (isScreenPainted()) {
        optimizeTable.call($grid, true);
      }
      ...
      if (gridSetting($grid, "force-desktop")) {
        if (gridsForceDesktop.length > 0) {
          gridsForceDesktop = gridsForceDesktop.add($grid);
        } else {
          gridsForceDesktop = $grid;
        }
        if (!$grid.hasClass("grid-list-mode")) {
        ...
        $grid
          ...
          .removeClass("answers-list")
          .addClass("answers-table");
      } else if (gridSetting($grid, "force-mobile")) {
        $grid
          .closest(".answers")
          .removeClass("answers-table")
          .addClass("answers-list");
      ...
      if (
        (gridSetting($grid, "force-mobile") ||
          gridSetting($grid, "list-mode")) &&
        $grid.hasClass("grid-table-mode")
      ) {
        ...
        gridSetting($grid, "force-desktop") &&
        $grid.hasClass("grid-list-mode")
      ) {
  ...
  // On window load and resize, equalize the widths of all labels positioned left of their form elements (lists only).
  // Uses a debouncer so function cannot run more than once every 100ms.
  function fixUnequalLabelWidths() {
    ...
    leftLegendListLabels
      ...
      .each(function () {
        var $qelements = $(this).find(leftLegendListLabels),
          maxWidth = 0;
  ...
  leftLegendListLabels = $(".cell-legend-left").find("label");
  if (leftLegendListLabels.length) {
  ...
  $(".question .answers:has(.fir-icon)").each(function () {
    var $me = $(this),
      ...
      $firDQs = $me
        ...
        .addClass("add-fir"),
      $inputs = $.merge(
        $me.find('input[type="radio"], input[type="checkbox"], select'),
        $firDQs
      ),
      $labels = $me.find("label");
    ...
    $inputs.each(function () {
      ...
      if ($input.is(".no-fir")) {
        ...
        $input.removeClass("fir-hidden").wrap("<div class='fir-select'></div>");
        return;
      } else if ($input.is(":radio")) {
        $input.after($firRadio.clone());
        $input.click(function (e) {
          $inputs
            .filter('[name="' + $input.attr("name") + '"]')
            .trigger("change");
        });
  ...
  var submitInput = $('input[type="submit"]'),
    ...
    primaryForm = document.getElementById("primary");

  if (primaryForm) {
    primaryForm.onsubmit = function () {
      var submitInput = $('input[type="submit"]');
      submitInput.attr("disabled", true);
      $("#btn_goback").attr("disabled", true);
      return true;
    };
  }
  ...
  if (
    !$.isFunction(submitInputOnclick) ||
    $.type(submitInputOnclick) === "undefined"
  ) {
  ...
  var singleGridAccordion = {
    init: function () {
      // Check if there are accordion elements questions on the page.
      var $accordions = $(".answers-accordion");

      // There could be more then one accordion on a page. Initialize each.
      $accordions.each(function () {
        var $accordion = $(this);
        var $rows = $accordion.find(".row.row-elements");

        singleGridAccordion.initRows($rows);
        singleGridAccordion.changeAnswerHandler($accordion, $rows);
        singleGridAccordion.headerToggle($accordion);
      });
    },
    // Initialize the rows, make sure the appropriate row is expanded if it is the first time the question is
    // shown, they submitted and received and error and clicked back to the question.
    initRows: function ($rows) {
      var unansweredRow = false;

      $rows.each(function () {
        var $row = $(this);
        var $answerInput = $row.find("input:checked");

        if ($answerInput.length !== 0) {
          var $accordionHeaderAnswer = $row.find(".accordion-header-answer");
          var answerLabel = $answerInput.parents(".cell").find("label").text();

          $accordionHeaderAnswer.find("span").text(answerLabel);
          $accordionHeaderAnswer.show();
        } else {
          // Open only the first accordion row that does not have an answer.
          if (!unansweredRow) {
            $row.find(".element").slideDown(200);
            unansweredRow = true;
          }
        }
      });
    },
    changeAnswerHandler: function ($accordion, $cols) {
      // Handles selecting row answer
      $accordion.find('input[type="radio"]').on("click", function () {
        // Row element
        var $currentRow = $(this).parents(".row");
        // Span in row that holds answer value
        var $rowHeaderAnswer = $currentRow.find(".accordion-header-answer");

        if ($("input:checked", $currentRow).length !== 0) {
          // Assign label to header if selected, empty it if nothing is select
          // Label text to go in row header.
          $rowHeaderAnswer
            .find("span")
            .html($(this).parents(".cell").find("label").text());

          // Close Accordion
          $currentRow.find(".element").slideUp(200);
          $currentRow.removeClass("open-accordion");
          $rowHeaderAnswer.show();

          $cols.each(function () {
            var $col = $(this);
            if (!$col.find(".accordion-header-answer span").text().length) {
              $col.find(".element").slideDown(200);
              $col.addClass("open-accordion");

              var $oeCell = $col.find(".element.awaitingOE");
              if ($oeCell.length === 1) {
                moveOE($oeCell, 0);
              }
              return false; // break
          ...
          $rowHeaderAnswer.hide().find("span").html("");
        }
      });
    },
    headerToggle: function ($accordion) {
      $(".row-legend", $accordion).on("click", function () {
        // question row
        var $row = $(this).parents(".row");

        // question options
        var $cols = $(".element", $row);

        // Hide all options
        $(".element", $accordion).slideUp(200);
        $accordion
          .find(".row.row-elements.open-accordion")
          .removeClass("open-accordion");

        // if the options for this row are not visible
        if (!$cols.is(":visible")) {
          // show the options for this row
          $cols.slideDown(200);
          $row.addClass("open-accordion");

          // Add OE input to the current cell
          var $oeCells = $cols.filter(".awaitingOE");

          if ($oeCells.length > 0) {
            $oeCells.each(function () {
              moveOE($(this), 0);
            });
          }
        }
