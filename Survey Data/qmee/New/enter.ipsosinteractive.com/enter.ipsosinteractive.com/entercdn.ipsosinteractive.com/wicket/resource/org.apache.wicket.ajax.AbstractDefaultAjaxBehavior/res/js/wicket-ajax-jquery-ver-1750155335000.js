...
(function (jQuery, undefined) {
    ...
    schedule: function (callback) {
      if (this.busy === false) {
        ...
        try {
          ...
          this.busy = false;
          Wicket.Log.error(
            "An error occurred while executing Ajax request:",
            exception
          );
        }
      } else {
        ...
        if (this.type === "s") {
          // stack/queue
          Wicket.Log.info(
            "%s - scheduling the callback to be executed when the previous request finish.",
            busyChannel
          );
          ...
          // drop
          Wicket.Log.info(
            "%s - dropping all previous scheduled callbacks and scheduling a new one to be executed when the current request finish.",
            busyChannel
          );
          this.callbacks = [];
          this.callbacks.push(callback);
          ...
          // active
          Wicket.Log.info(
            "%s - ignoring the Ajax call because there is a running request.",
            busyChannel
          );
        }
        return null;
  ...
  Wicket.Ajax.Call.prototype = {
    ...
    ajax: function (attrs) {
      ...
      var res = Wicket.channelManager.schedule(
        attrs.ch,
    ...
    doAjax: function (attrs) {
      var // the headers to use for each Ajax request
        headers = {
          "Wicket-Ajax": "true",
        ...
        defaultPrecondition = [
          function (attributes) {
            return (
              self._isPresent(attributes.c) && self._isPresent(attributes.f)
            );
          },
        ],
      ...
      if (Wicket.Focus.lastFocusId) {
        ...
        headers["Wicket-FocusedElementId"] = Wicket.Form.encode(
      ...
      if (jQuery.isArray(preconditions)) {
        ...
        for (var p = 0; p < preconditions.length; p++) {
          ...
          if (result === false) {
            Wicket.Log.info(
              "Ajax request stopped because of precondition check, url: %s",
              attrs.u
            );
            self.done(attrs);
            return false;
    ...
    processAjaxResponse: function (data, textStatus, jqXHR, context) {
      if (jqXHR.readyState === 4) {
        ...
        if (
          ...
          Wicket.Log.info(
            "Received ajax response (%s characters)",
    ...
    loadedCallback: function (envelope, context) {
      ...
      try {
        ...
        if (stepIndexOfLastReplacedComponent !== -1) {
          this.processFocusedComponentReplaceCheck(
            steps,
    ...
    success: function (context) {
      context.steps.push(
        jQuery.proxy(function (notify) {
          ...
          Wicket.Event.publish(
            Wicket.Event.Topic.AJAX_CALL_SUCCESS,
            attrs,
            null,
            null,
            "success"
    ...
    failure: function (context, jqXHR, errorMessage, textStatus) {
      context.steps.push(
        jQuery.proxy(function (notify) {
          if (errorMessage) {
            Wicket.Log.error(
              "Wicket.Ajax.Call.failure: Error while parsing response: %s",
              errorMessage
            );
          }
          var attrs = context.attrs;
          ...
          Wicket.Event.publish(
            Wicket.Event.Topic.AJAX_CALL_FAILURE,
            attrs,
            jqXHR,
            errorMessage,
            textStatus
    ...
    processComponent: function (context, node) {
      context.steps.push(function (notify) {
        ...
        if (isUndef(element)) {
          Wicket.Log.error(
            "Wicket.Ajax.Call.processComponent: Component with id '%s' was not found while trying to perform markup update. " +
    ...
    processFocusedComponentReplaceCheck: function (
      steps,
  ...
  jQuery.extend(true, Wicket, {
    ...
    Form: {
      ...
      serializeElement: function (element, serializeRecursively) {
        ...
        if (
          ...
          element.id &&
          Wicket.Form.excludeFromAjaxSerialization[element.id] === "true"
        ) {
          return [];
        }
        ...
        if (tag === "select") {
          ...
          if (serializeRecursively) {
            var elements = nodeListToArray(
              element.getElementsByTagName("input")
            );
            elements = elements.concat(
              nodeListToArray(element.getElementsByTagName("select"))
            );
            elements = elements.concat(
              nodeListToArray(element.getElementsByTagName("textarea"))
            );
            ...
            for (var i = 0; i < elements.length; ++i) {
              ...
              if (el.name && el.name !== "") {
                result = result.concat(
                  Wicket.Form.serializeElement(el, serializeRecursively)
                );
              }
            }
          }
        ...
        jQuery.each(attrs.e, function (idx, evt) {
          Wicket.Event.add(
            ...
            function (jqEvent, data) {
              ...
              Wicket.Event.publish(
                Wicket.Event.Topic.AJAX_CALL_INIT,
                attributes
              );
              ...
              if (throttlingSettings) {
                ...
                throttler.throttle(
                  throttlingSettings.id,
                  throttlingSettings.d,
                  Wicket.bind(function () {
    ...
    Head: {
            ...
            if (src !== null && src !== "") {
              ...
              if (typeof scriptDomNode.onload !== "undefined") {
                ...
                scriptDomNode.onreadystatechange = function () {
                  if (
                    scriptDomNode.readyState === "loaded" ||
                    scriptDomNode.readyState === "complete"
                  ) {
                    onScriptReady();
                  }
                };
              ...
              try {
                ...
                Wicket.Log.error(
                  "Ajax.Call.processEvaluation: Exception evaluating javascript: %s",
                  text,
                  exception
                );
              } finally {
      ...
      addElement: function (element, remove) {
        var headItems = document.querySelector(
          'head meta[name="wicket.header.items"]'
        );
        if (headItems) {
    ...
    Focus: {
      ...
      focusout: function (event) {
        ...
        if (target && WF.lastFocusId === target.id) {
          ...
          if (WF.refocusLastFocusedComponentAfterResponse) {
            ...
            Wicket.Log.info(
              "focus removed from '%s' but ignored because of component replacement",
              id
            );
          } else {
            WF.lastFocusId = null;
      ...
      setFocusOnId: function (id) {
        ...
        if (id) {
          ...
          WF.refocusLastFocusedComponentAfterResponse = false;
          Wicket.Log.info(
            "refocus focused component after request stopped from server side"
          );
        }
      },
      ...
      checkFocusedComponentReplaced: function () {
        ...
        if (WF.refocusLastFocusedComponentAfterResponse) {
          ...
          if (focusedElement) {
            if (
              typeof focusedElement.wasFocusedBeforeComponentReplacements !==
              "undefined"
            ) {
      ...
      requestFocus: function () {
        ...
        if (WF.refocusLastFocusedComponentAfterResponse && WF.lastFocusId) {
          ...
          if (toFocus) {
            ...
            WF.lastFocusId = "";
            Wicket.Log.info(
              "Couldn't set focus on element with id '%s' because it is not in the page anymore",
              WF.lastFocusId
            );
          }
    ...
    Event: {
      ...
      add: function (element, type, fn, data, selector) {
        if (type === "domready") {
          ...
          if (!el && Wicket.Log) {
            Wicket.Log.error(
              "Cannot bind a listener for event '%s' because the element is not in the DOM",
              type,
              element
            );
          }
