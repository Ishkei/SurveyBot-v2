...
(function (root, factory) {
    ...
    function getUTCDate_async(csrfkey) {
        ...
        getUTCDate_async_(csrfkey).then(function (data) {
    ...
    function validateTateichi(mtmTableElms, mtsTableElms) {
        ...
        isValidToMTM = $mtmTables.filter(function (index, tables) {
            var ERROR = true,
                SUCCESS = !ERROR,
            ...
            return ERROR == ($inputList
                ...
                .filter(function (qqIdAndItemId) {
                    return $('input[id^='+qqIdAndItemId+']:checked', tables).length != selectionCheckedCount;
                })
                .length == 0);
        ...
        isValidToMTS = $mtsTables.filter(function (index, tables) {
            var ERROR = true,
                SUCCESS = !ERROR,
            ...
            hasAnyUnanswered = qqIdAndItemIdList
                .filter(function (qqIdAndItemId) {
                    return $inputList.filter('input[name='+qqIdAndItemId+']:checked').length == 0;
                }).length > 0;
            if (hasAnyUnanswered) return SUCCESS;

            // 全ての項目で同じ選択肢を選んでいる場合はエラー
            return ERROR == (qqIdAndItemIdList
                .map(function (qqIdAndItemId) {
                    return $inputList.filter('input[name='+qqIdAndItemId+']:checked')[0].value;
                })
                .filter(array_unique)
                .length == 1
            );
    ...
    function sendBugReport(BD, params) {
        ...
        if (params.p1 == true) { // document.getElementById("p1").checked
            data += '&p1=1';
        }
        if (params.p2 == true) { // document.getElementById("p2").checked
            data += '&p2=1';
        }
        if (params.p3 == true) { // document.getElementById("p3").checked
            data += '&p3=1';
        }
        if (params.p4 == true) { // document.getElementById("p4").checked
            data += '&p4=1';
        }
        if (params.p5 == true) { // document.getElementById("p5").checked
            data += '&p5=1';
        }

        data += '&ptext=' + encodeURIComponent(params.ptext); // document.getElementById("ptext").value

        if (params.s1 == true) { // document.getElementById("s1").checked
            data += '&s1=1';
        }

        if (params.s2 == true) { // document.getElementById("s2").checked
            data += '&s2=1';
        }
    ...
    function getDeviceflag() {
        ...
        if (navigator.userAgentData) {
            ...
            if (ua.match(/iPhone/i)) {
                ...
                return 3;
            } else if (ua.match(/Mozilla\/5.0 (compatible; MSIE 10.0; Windows NT 6.2; Trident\/6.0)/i)) {
                return 3;
            } else if (ua.match(/Mozilla\/5.0 (compatible; MSIE 10.0; Windows NT 6.2; Trident\/6.0; Touch)/i)) {
                return 3;
            } else {
                return 1;
            }
    ...
    function getBrandsToString() {
        ...
        if (navigator.userAgentData) {
            for(let i = 0; i < navigator.userAgentData.brands.length; i++ ) {
                ua += (i == 0 ? "" : ",") + '"' + navigator.userAgentData.brands[i].brand + '";v="' + navigator.userAgentData.brands[i].version + '"';
            }
        } else {
