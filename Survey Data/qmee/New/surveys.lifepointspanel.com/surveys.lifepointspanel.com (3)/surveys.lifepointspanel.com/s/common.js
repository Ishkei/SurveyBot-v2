...
function renameKeyPress(event, action, idx) {
  ...
  /* IE/NS compat */
  if (event.which) {
    /* Opera will supply both but uses .which ! */
    key = event.which;
  ...
  for (var f = 0; f < d.forms.length; f++) {
    ...
    for (var e = 0; e < frm.elements.length; e++) {
      ...
      if (tag === "input" || tag === "textarea") {
        ...
        el.selectedIndex =
          1 + Math.floor(Math.random() * (el.options.length - 1));
      }
    }
  }
}
    ...
    if (slotFound) {
      ...
      if (slotFound.item && slotFound.item !== ddObject) {
        ...
        if (ddObject.slot) {
          ...
          setXY(
            slotFound.item,
            slotFound.item.originalX,
            slotFound.item.originalY
          );
...
setFolderName = function setFolderName(here, folder, text) {
  window.location =
    here + "?showFolders=1&folder=" + folder + "&setFolderName=" + text;
};
...
function extractCookie(name) {
  var cookie = document.cookie.match(
    new RegExp("(^|;)\\s*" + encodeURIComponent(name) + "=([^;\\s]*)")
  );
...
jsonAjax = function jsonAjax(fun, args, errback) {
  return simpleAjax(
    appendSurvey(fun, args.survey || args.path),
    Object.toJSON(args),
    errback
  );
};
...
function dashboardSelectSplit(path, tab, name) {
  ...
  if (sendForm) {
    ...
    window.location =
      "/rep/" + path + ":dashboard?tab=" + tab + "&split=" + name;
  }
}
...
function mailqueueShowAll() {
  $A(document.getElementsByClassName("collapsed")).each(function (el) {
    el.style.display = "";
  });
...
var ImageSwap = {
  enable: function () {
    ...
    // preload images
    $$("IMG.mouseouver").each(function (el) {
      new Image("mo-" + el.src);
    });
  },
...
Survey = {
  ...
  disableDisabled: function () {
    ...
    $$(".disable-inside INPUT").each(function (el) {
      el.disabled = 1;
      el.parentNode.onmouseover = null;
      el.parentNode.onmouseout = null;
    });
    $$("TD.element INPUT").each(function (el) {
  ...
  handleOpen: function (openName, clickableName, clickableValue) {
    addHandler(function () {
      ...
      for (var i = 0; i < b.length; i++) {
        ...
        if (window.jQuery) {
          $(b[i]).change(btnHandler);
        }
        if (
          type === "checkbox" ||
          (type === "radio" && b[i].value === clickableValue)
        ) {
          if (!window.jQuery) {
      ...
      o.onkeypress = o.onblur = o.onkeyup = populateBuddy;
    });
  },
  ...
  capturePreciseTime: function () {
    ...
    var now = new Date();
    if (button && tf) {
      Survey.addListener(button, "click", function () {
  ...
  recordPageTiming: function (path, uuid, ident) {
    ...
    var url =
      ...
      path +
      "?v2timing=" +
      [uuid, ident, response, loaded, dom].join(",");
    $.get(url);
  },
};
