...
function E(r, e) {
  ...
  return (
    I(r)
      ? ((t = `Binary data of length ${r.byteLength}`),
        e && (t += `. Content: '${K(r)}'`))
      : typeof r == "string" &&
...
async function L(r, e, t, n, o, i) {
  const c = {},
    [l, a] = v();
  (c[l] = a),
    r.log(
      s.Trace,
      `(${e} transport) sending data. ${E(o, i.logMessageContent)}.`
    );
  const h = I(o) ? "arraybuffer" : "text",
    g = await t.post(n, {
      content: o,
      headers: { ...c, ...i.headers },
      responseType: h,
...
class P {
  ...
  log(e, t) {
    if (e >= this._minLevel) {
      const n = `[${new Date().toISOString()}] ${s[e]}: ${t}`;
      switch (e) {
        case s.Critical:
        case s.Error:
          this.out.error(n);
...
function v() {
  ...
  return p.isNode && (r = "User-Agent"), [r, Q(J, Y(), q(), Z())];
}
function Q(r, e, t, n) {
...
function Z() {
  if (p.isNode) return process.versions.node;
}
function q() {
...
class te extends $ {
  constructor(e) {
    ...
    } else this._fetchType = fetch.bind(ee());
  ...
  async send(e) {
    if (e.abortSignal && e.abortSignal.aborted) throw new b();
    ...
    try {
      ...
      throw (
        n || (this._logger.log(s.Warning, `Error from HTTP request. ${a}.`), a)
      );
    } finally {
      o && clearTimeout(o), e.abortSignal && (e.abortSignal.onabort = null);
    }
    if (!i.ok) {
      const a = await W(i, "text");
      throw new y(a || i.statusText, i.status);
    }
    const l = await W(i, e.responseType);
    return new j(i.status, i.statusText, l);
  }
  getCookieString(e) {
    let t = "";
    return (
...
class oe extends $ {
  constructor(e) {
    if ((super(), typeof fetch < "u" || p.isNode)) this._httpClient = new te(e);
    else if (typeof XMLHttpRequest < "u") this._httpClient = new ne(e);
...
class m {
  ...
  static parse(e) {
    ...
    const t = e.split(m.RecordSeparator);
    return t.pop(), t;
  }
}
  ...
  parseHandshakeResponse(e) {
    ...
    if (I(e)) {
      const l = new Uint8Array(e),
        a = l.indexOf(m.RecordSeparatorCode);
      if (a === -1) throw new Error("Message is incomplete.");
      const h = a + 1;
      (t = String.fromCharCode.apply(
        null,
        Array.prototype.slice.call(l.slice(0, h))
      )),
        (n = l.byteLength > h ? l.slice(h).buffer : null);
    } else {
      const l = e,
        a = l.indexOf(m.RecordSeparator);
      if (a === -1) throw new Error("Message is incomplete.");
      const h = a + 1;
      (t = l.substring(0, h)), (n = l.length > h ? l.substring(h) : null);
    }
    const o = m.parse(t),
      i = JSON.parse(o[0]);
    if (i.type)
...
(function (r) {
  (r[(r.Invocation = 1)] = "Invocation"),
    (r[(r.StreamItem = 2)] = "StreamItem"),
    (r[(r.Completion = 3)] = "Completion"),
    (r[(r.StreamInvocation = 4)] = "StreamInvocation"),
    (r[(r.CancelInvocation = 5)] = "CancelInvocation"),
    (r[(r.Ping = 6)] = "Ping"),
    (r[(r.Close = 7)] = "Close");
})(u || (u = {}));
class se {
...
class R {
  constructor(e, t, n, o) {
    (this._nextKeepAlive = 0),
      ...
      f.isRequired(e, "connection"),
      f.isRequired(t, "logger"),
      f.isRequired(n, "protocol"),
      (this.serverTimeoutInMilliseconds = ie),
      (this.keepAliveIntervalInMilliseconds = ce),
      (this._logger = t),
      (this._protocol = n),
      (this.connection = e),
      (this._reconnectPolicy = o),
      (this._handshakeProtocol = new re()),
      (this.connection.onreceive = (i) => this._processIncomingData(i)),
      (this.connection.onclose = (i) => this._connectionClosed(i)),
      (this._callbacks = {}),
      (this._methods = {}),
      ...
      (this._connectionState = d.Disconnected),
      (this._connectionStarted = !1),
      (this._cachedPingMessage = this._protocol.writeMessage({ type: u.Ping }));
  }
  ...
  set baseUrl(e) {
    if (
      this._connectionState !== d.Disconnected &&
      this._connectionState !== d.Reconnecting
    )
      throw new Error(
  ...
  async _startWithStateTransitions() {
    ...
    (this._connectionState = d.Connecting),
      this._logger.log(s.Debug, "Starting HubConnection.");
    try {
      await this._startInternal(),
        p.isBrowser &&
          window.document.addEventListener("freeze", this._freezeEventListener),
        (this._connectionState = d.Connected),
    ...
    return (
      (l.cancelCallback = () => {
        ...
        return (
          ...
          c.then(() => this._sendWithProtocol(a))
        );
      }),
      (this._callbacks[i.invocationId] = (a, h) => {
        if (h) {
          l.error(h);
          return;
        } else
      ...
      (c = this._sendWithProtocol(i).catch((a) => {
        l.error(a), delete this._callbacks[i.invocationId];
      })),
      this._launchStreams(n, c),
      l
    );
  }
  _sendMessage(e) {
  ...
  send(e, ...t) {
    const [n, o] = this._replaceStreamingParams(t),
      i = this._sendWithProtocol(this._createInvocation(e, t, !0, o));
    return this._launchStreams(n, i), i;
  }
  invoke(e, ...t) {
    const [n, o] = this._replaceStreamingParams(t),
      i = this._createInvocation(e, t, !1, o);
    return new Promise((l, a) => {
      this._callbacks[i.invocationId] = (g, C) => {
        if (C) {
          a(C);
          return;
        } else
          g &&
            (g.type === u.Completion
              ? g.error
                ? a(new Error(g.error))
                : l(g.result)
              : a(new Error(`Unexpected message type: ${g.type}`)));
      };
      const h = this._sendWithProtocol(i).catch((g) => {
        a(g), delete this._callbacks[i.invocationId];
      });
          ...
          case u.Completion: {
            ...
            if (o) {
              ...
              try {
                ...
                this._logger.log(
                  s.Error,
                  `Stream callback threw error: ${A(i)}`
                );
              }
            }
            break;
    ...
    if (
      ...
      e < 0 && (e = 0),
        (this._pingServerHandle = setTimeout(async () => {
          if (this._connectionState === d.Connected)
            try {
              await this._sendMessage(this._cachedPingMessage);
            } catch {
              this._cleanupPingTimer();
  ...
  async _invokeClientMethod(e) {
    ...
    for (const h of o)
      try {
        const g = c;
        (c = await h.apply(this, e.arguments)),
          i &&
            c &&
            g &&
            (this._logger.log(
    ...
    a
      ...
      : i
      ? (l
          ? (a = this._createCompletionMessage(e.invocationId, `${l}`, null))
          : c !== void 0
          ? (a = this._createCompletionMessage(e.invocationId, null, c))
          : (this._logger.log(
              s.Warning,
              `No result given for '${t}' method and invocation ID '${e.invocationId}'.`
            ),
  ...
  _connectionClosed(e) {
    this._logger.log(
      ...
      this._connectionState === d.Disconnecting
        ...
        : this._connectionState === d.Connected && this._reconnectPolicy
        ? this._reconnect(e)
        : this._connectionState === d.Connected && this._completeClose(e);
  }
  _completeClose(e) {
  ...
  async _reconnect(e) {
    ...
    let n = 0,
      ...
      i = this._getNextRetryDelay(n++, 0, o);
    if (i === null) {
      this._logger.log(
        s.Debug,
    ...
    if (
      ...
      try {
        ...
        this._logger.log(
          s.Error,
          `An onreconnecting callback called with error '${e}' threw error '${c}'.`
        );
      }
    ...
    for (; i !== null; ) {
      if (
        (this._logger.log(
          s.Information,
          `Reconnect attempt number ${n} will start in ${i} ms.`
        ),
        await new Promise((c) => {
          this._reconnectDelayHandle = setTimeout(c, i);
        }),
        (this._reconnectDelayHandle = void 0),
        this._connectionState !== d.Reconnecting)
      ) {
      ...
      try {
        if (
          ...
          try {
            ...
            this._logger.log(
              s.Error,
              `An onreconnected callback called with connectionId '${this.connection.connectionId}; threw error '${c}'.`
            );
          }
        return;
      } catch (c) {
        if (
          (this._logger.log(
            s.Information,
            `Reconnect attempt failed because of error '${c}'.`
          ),
          this._connectionState !== d.Reconnecting)
        ) {
          this._logger.log(
            ...
            this._connectionState === d.Disconnecting && this._completeClose();
          return;
        }
        (o = c instanceof Error ? c : new Error(c.toString())),
          (i = this._getNextRetryDelay(n++, Date.now() - t, o));
      }
    }
    this._logger.log(
      s.Information,
    ...
    (this._callbacks = {}),
      Object.keys(t).forEach((n) => {
        ...
        try {
          ...
          this._logger.log(
            s.Error,
            `Stream 'error' callback called with '${e}' threw error: ${A(i)}`
          );
        }
      });
  }
  ...
  _replaceStreamingParams(e) {
    ...
    for (let o = 0; o < e.length; o++) {
      ...
      if (this._isObservable(i)) {
        ...
        this._invocationId++, (t[c] = i), n.push(c.toString()), e.splice(o, 1);
      }
    }
    return [t, n];
  }
...
const ae = [0, 2e3, 1e4, 3e4, null];
class N {
  constructor(e) {
    this._retryDelays = e !== void 0 ? [...e, null] : ae;
  }
...
class le extends $ {
  ...
  _setAuthorizationHeader(e) {
    e.headers || (e.headers = {}),
      this._accessToken
        ? (e.headers[S.Authorization] = `Bearer ${this._accessToken}`)
        : this._accessTokenFactory &&
          e.headers[S.Authorization] &&
          delete e.headers[S.Authorization];
  }
  getCookieString(e) {
...
(function (r) {
  (r[(r.None = 0)] = "None"),
    (r[(r.WebSockets = 1)] = "WebSockets"),
    (r[(r.ServerSentEvents = 2)] = "ServerSentEvents"),
    (r[(r.LongPolling = 4)] = "LongPolling");
})(_ || (_ = {}));
var w;
(function (r) {
...
class M {
  ...
  async connect(e, t) {
    if (
      (f.isRequired(e, "url"),
      f.isRequired(t, "transferFormat"),
      f.isIn(t, w, "transferFormat"),
      (this._url = e),
    ...
    t === w.Binary && (c.responseType = "arraybuffer");
    const l = `${e}&_=${Date.now()}`;
    this._logger.log(s.Trace, `(LongPolling transport) polling: ${l}.`);
    const a = await this._httpClient.get(l, c);
    a.statusCode !== 200
      ? (this._logger.log(
          s.Error,
          `(LongPolling transport) Unexpected response code: ${a.statusCode}.`
        ),
        (this._closeError = new y(a.statusText || "", a.statusCode)),
        (this._running = !1))
      : (this._running = !0),
      (this._receiving = this._poll(this._url, c));
  }
  async _poll(e, t) {
    try {
      for (; this._running; )
        try {
          ...
          o.statusCode === 204
            ...
            ? (this._logger.log(
              ...
              (this._closeError = new y(o.statusText || "", o.statusCode)),
              (this._running = !1))
            : o.content
            ? (this._logger.log(
                s.Trace,
                `(LongPolling transport) data received. ${E(
                  o.content,
...
class ge {
    ...
    return (
      ...
      new Promise((n, o) => {
        ...
        if (p.isBrowser || p.isWebWorker)
          c = new this._options.EventSource(e, {
            withCredentials: this._options.withCredentials,
          });
        else {
  ...
  async send(e) {
    return this._eventSource
      ? L(this._logger, "SSE", this._httpClient, this._url, e, this._options)
      : Promise.reject(
          new Error("Cannot send until the transport is connected")
        );
  }
...
class ue {
      ...
      new Promise((o, i) => {
        ...
        if (p.isNode || p.isReactNative) {
          const h = {},
            [g, C] = v();
          (h[g] = C),
            n && (h[S.Authorization] = `Bearer ${n}`),
            l && (h[S.Cookie] = l),
            (c = new this._webSocketConstructor(e, void 0, {
        ...
        c || (c = new this._webSocketConstructor(e)),
          t === w.Binary && (c.binaryType = "arraybuffer"),
          (c.onopen = (h) => {
          ...
          (c.onerror = (h) => {
            let g = null;
            typeof ErrorEvent < "u" && h instanceof ErrorEvent
              ? (g = h.error)
              : (g = "There was an error with the transport"),
              this._logger.log(s.Information, `(WebSockets transport) ${g}.`);
          }),
          (c.onmessage = (h) => {
            if (
              (this._logger.log(
                s.Trace,
                `(WebSockets transport) data received. ${E(
                  h.data,
  ...
  send(e) {
    return this._webSocket &&
      ...
      ? (this._logger.log(
          s.Trace,
          `(WebSockets transport) sending data. ${E(
            e,
            this._logMessageContent
          )}.`
        ),
...
class de {
    ...
    if (p.isNode && typeof require < "u") {
      const i =
        typeof __webpack_require__ == "function"
          ? __non_webpack_require__
          : require;
      (n = i("ws")), (o = i("eventsource"));
    }
    !p.isNode && typeof WebSocket < "u" && !t.WebSocket
      ? (t.WebSocket = WebSocket)
      : p.isNode && !t.WebSocket && n && (t.WebSocket = n),
      !p.isNode && typeof EventSource < "u" && !t.EventSource
        ? (t.EventSource = EventSource)
        : p.isNode && !t.EventSource && typeof o < "u" && (t.EventSource = o),
      (this._httpClient = new le(
        t.httpClient || new oe(this._logger),
  ...
  async start(e) {
    if (
      ((e = e || w.Binary),
      f.isIn(e, w, "transferFormat"),
      this._logger.log(
        s.Debug,
        `Starting connection with transfer format '${w[e]}'.`
      ),
      this._connectionState !== "Disconnected")
    )
      return Promise.reject(
  ...
  send(e) {
    return this._connectionState !== "Connected"
      ...
      : (this._sendQueue || (this._sendQueue = new x(this.transport)),
  ...
  async _startInternal(e) {
    ...
    try {
      if (this._options.skipNegotiation)
        if (this._options.transport === _.WebSockets)
      ...
      this.transport instanceof M && (this.features.inherentKeepAlive = !0),
        this._connectionState === "Connecting" &&
          (this._logger.log(
            s.Debug,
  ...
  async _getNegotiationResponse(e) {
    ...
    try {
      const c = await this._httpClient.post(i, {
        content: "",
      ...
      if (c.statusCode !== 200)
        return Promise.reject(
          new Error(
            `Unexpected status code returned from negotiate '${c.statusCode}'`
          )
        );
      ...
      let l = "Failed to complete negotiation with the server: " + c;
      return (
        c instanceof y &&
          c.statusCode === 404 &&
          (l =
  ...
  async _createTransport(e, t, n, o) {
    ...
    const c = [],
      l = n.availableTransports || [];
    let a = n;
    for (const h of l) {
      const g = this._resolveTransportOrError(h, t, o);
      if (g instanceof Error) c.push(`${h.transport} failed:`), c.push(g);
      else if (this._isITransport(g)) {
        if (((this.transport = g), !a)) {
          try {
        ...
        try {
          ...
          if (
            (this._logger.log(
              s.Error,
              `Failed to start the transport '${h.transport}': ${C}`
            ),
            (a = void 0),
            c.push(new F(`${h.transport} failed: ${C}`, _[h.transport])),
            this._connectionState !== "Connecting")
          ) {
            const H = "Failed to select transport before stop() was called.";
            return this._logger.log(s.Debug, H), Promise.reject(new b(H));
          }
        }
      }
    }
    return c.length > 0
      ? Promise.reject(
          new X(
            `Unable to connect to the server with any of the available transports. ${c.join(
              " "
            )}`,
            c
          )
  ...
  _resolveTransportOrError(e, t, n) {
    ...
    if (_e(t, o))
      if (e.transferFormats.map((c) => w[c]).indexOf(n) >= 0) {
        if (
          (o === _.WebSockets && !this._options.WebSocket) ||
          (o === _.ServerSentEvents && !this._options.EventSource)
        )
          return (
            this._logger.log(
              s.Debug,
              `Skipping transport '${_[o]}' because it is not supported in your environment.'`
            ),
            new O(`'${_[o]}' is not supported in your environment.`, o)
          );
        this._logger.log(s.Debug, `Selecting transport '${_[o]}'.`);
        try {
        ...
        return (
          this._logger.log(
            s.Debug,
            `Skipping transport '${_[o]}' because it does not support the requested transfer format '${w[n]}'.`
          ),
          new Error(`'${_[o]}' does not support ${w[n]}.`)
        );
    else
      return (
        this._logger.log(
          s.Debug,
          `Skipping transport '${_[o]}' because it was disabled by the client.`
        ),
        new B(`'${_[o]}' is disabled by the client.`, o)
      );
  }
  _isITransport(e) {
  ...
  _resolveUrl(e) {
    if (e.lastIndexOf("https://", 0) === 0 || e.lastIndexOf("http://", 0) === 0)
      return e;
    if (!p.isBrowser) throw new Error(`Cannot resolve '${e}'.`);
  ...
  _resolveNegotiateUrl(e) {
    const t = e.indexOf("?");
    let n = e.substring(0, t === -1 ? e.length : t);
    return (
      n[n.length - 1] !== "/" && (n += "/"),
      (n += "negotiate"),
      (n += t === -1 ? "" : e.substring(t)),
      n.indexOf("negotiateVersion") === -1 &&
        ((n += t === -1 ? "?" : "&"),
...
class x {
  constructor(e) {
    (this._transport = e),
      ...
      (this._transportResult = new T()),
      (this._sendLoopPromise = this._sendLoop());
  }
  send(e) {
    return (
      this._bufferData(e),
      this._transportResult || (this._transportResult = new T()),
  ...
  static _concatBuffers(e) {
    const t = e.map((i) => i.byteLength).reduce((i, c) => i + c),
      n = new Uint8Array(t);
    let o = 0;
    for (const i of e) n.set(new Uint8Array(i), o), (o += i.byteLength);
    return n.buffer;
  }
}
class T {
...
class fe {
  constructor() {
    (this.name = pe), (this.version = 1), (this.transferFormat = w.Text);
  }
  parseMessages(e, t) {
    if (typeof e != "string")
      throw new Error(
    ...
    for (const i of n) {
      ...
      if (typeof c.type != "number") throw new Error("Invalid payload.");
      switch (c.type) {
        case u.Invocation:
        ...
        default:
          t.log(
            s.Information,
            "Unknown message type '" + c.type + "' ignored."
          );
          continue;
      }
      o.push(c);
...
class Se {
  configureLogging(e) {
    if ((f.isRequired(e, "logging"), be(e))) this.logger = e;
    else if (typeof e == "string") {
      const t = me(e);
      this.logger = new P(t);
    } else this.logger = new P(e);
    return this;
  }
  withUrl(e, t) {
    return (
      ...
      (this.url = e),
      typeof t == "object"
        ? (this.httpConnectionOptions = { ...this.httpConnectionOptions, ...t })
  ...
  build() {
    ...
    const t = new de(this.url, e);
    return R.create(
      t,
      this.logger || k.instance,
      this.protocol || new fe(),
...
export {
  b as AbortError,
  oe as DefaultHttpClient,
  $ as HttpClient,
  y as HttpError,
  j as HttpResponse,
  _ as HttpTransportType,
  R as HubConnection,
  Se as HubConnectionBuilder,
  d as HubConnectionState,
  fe as JsonHubProtocol,
  s as LogLevel,
  u as MessageType,
  k as NullLogger,
  se as Subject,
