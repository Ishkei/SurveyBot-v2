...
$.extend({
  checkFirstChild: function (obj, event) {
    var children, x, node, tag, isFir, isFirQ, $eventTarget;
    ...
    if (tag === "input" || tag === "a" || isFir) {
      ...
      tag === "label" &&
      $eventTarget.attr("for") &&
      !$eventTarget.parent().hasClass("cell-text")
    ) {
...
window.setupExclusive = function (grouping, elementName) {
  ...
  if (grouping === "cols") {
    var elementName_substr = elementName.substr(
        0,
        elementName.lastIndexOf(".")
      ),
      $exclusiveElement_group = jQuery(
        'input[name^="' + elementName_substr + '."]'
      );
