...
jQuery(document).ready(function () {
    ...
    var resizeTimer, resizeTimer2, paintTimer, paintTimerCount = 0, unevenLabelTimer = null, leftLegendListLabels, clickTimer;
    ...
    function handleOEKeypress(){
        ...
        var $oe = $(this);
        if ($oe.hasClass('mobileOE')) { // Skip if OE isn't approved for jumping.
    ...
    function moveOE($cell, speed) {
        ...
        var currRowIsOpen = $cell.parents('.row.row-elements.open-accordion').length > 0;
        var $openAccordions = $cell.parents('.answers.answers-accordion').find('.row.row-elements.open-accordion');


        if (!$cell.hasClass('awaitingOE')) { return; }
        if ($openAccordions.length !== 0 && !currRowIsOpen) { return; }
    ...
    function optimizeOE(telement, isMobile) {
                    ...
                    // Scenario group-by-col multi-column
                        ...
                        $oe.closest('.row').find('.element').each(function(){
                            ...
                            if (!$cell.hasClass('supportsOE')){
                                ...
                                $cell.find('.cell-text').append('<span class="mobile-oe-legend" data-for="' + $oe.attr('id') + '">' + $oe.val() + '</span>');
                            } else {
        ...
        // Grids with problematic col-legends
            ...
            if (isMobile || telement.hasClass('grid-list-mode')) {
                ...
                var $uniqueOEs = telement.find('.col-legend .oe-top, .col-legend .oe-bottom')
                    .each(function() {
                ...
                $uniqueOEs.each(function(){
                    ...
                    var $oe = $(this).addClass('mobileOE'), index = $oe.closest('.row').find('.cell').index($oe.parent()), first = true;
                    // Bind the keypress event if we haven't already.
                    if (!$oe.hasClass('supportsMobileOE')) {
                        $oe.addClass('supportsMobileOE').keyup(handleOEKeypress);
                    }
            ...
            // Otherwise, undo any previous moves.
                ...
                telement.find('.mobileOE').each(function(){
                    ...
                    var $oe = $(this).removeClass('mobileOE'), index = $oe.closest('.row').find('.cell').index($oe.closest('.cell'));
                    // Loop through each row with element cells.
    ...
    $(".grid").each(function (index) {
        ...
        // Handle Non-Optimized grids
            ...
            if (gridSetting($grid, "force-desktop")) {
                ...
                $grid.closest('.answers').removeClass("answers-list").addClass("answers-table");
            } else if (gridSetting($grid, "force-mobile")) {
                $grid.closest('.answers').removeClass("answers-table").addClass("answers-list");
             }
    ...
    function fixUnequalLabelWidths() {
        ...
        leftLegendListLabels.css('display', 'inline-block').closest('.answers-list').each(function(){
            var $qelements = $(this).find(leftLegendListLabels), maxWidth = 0;
    ...
    $(".question .answers:has(.fir-icon)").each(function () {
        ...
        $inputs.each(function() {
            ...
            if ($input.is('.no-fir')) {
                ...
                $input.click(function(e) {
                    $inputs.filter('[name="' + $input.attr('name') + '"]').trigger('change');
                });
        ...
        $labels.click(function (event) {
            ...
            if ($input.is(':radio') && !$input.prop('disabled') && $input.prop('checked')) {
    ...
    var singleGridAccordion = {
        ...
        changeAnswerHandler: function($accordion, $cols) {
            ...
            $accordion.find('input[type="radio"]').on('click', function () {
                ...
                if ($('input:checked', $currentRow).length !== 0) {
                    ...
                    $rowHeaderAnswer.find('span').html($(this).parents('.cell').find('label').text());
        ...
        headerToggle: function ($accordion) {
            $('.row-legend', $accordion).on('click', function () {
                ...
                $accordion.find(".row.row-elements.open-accordion").removeClass('open-accordion');
