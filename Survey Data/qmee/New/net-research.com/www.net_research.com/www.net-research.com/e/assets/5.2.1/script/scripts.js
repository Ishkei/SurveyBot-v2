...
function mmAddRankButton(qid, maxrank) {
    ...
    for(i=1; i<=mmRankButton.maxrank[qid]; i++) {
        ...
        if(frm.elements[qid + mmRankButton.delimiter + i].value > 0){
            frm.elements['b' + qid + mmRankButton.delimiter + frm.elements[qid + mmRankButton.delimiter + i].value].value = i;
		}
		}
	}
}
...
function mmSwitchRank(id) {
    ...
    if(frm.elements['b'+id].value > 0) {
		frm.elements[qid + mmRankButton.delimiter + frm.elements['b'+id].value].value = '';
...
function funcRotate(qid){
	...
	jQuery(elm0).each(function(){
		sa[cnt] = jQuery("div#qa-q" + qid + " td.mt-hyoken").height() - jQuery("div#qa-q" + qid + " td.mt-hyoto div.vertical:eq("+cnt+")").height();
		cnt++;
	});
	...
	if(!SharedFunction.isIE()){
		...
		*/
		jQuery("#qa-q"+ qid +" td.mt-hyoto:has(div.vertical)").css("height",wid + "px")
															  .css("width",hei1 + "px");
	}
}
	...
	function checkTateichi(){
		...
		if(nTateichiAlert == 0 && !SharedFunction.validateTateichi(mtmTableElms, mtsTableElms)){
