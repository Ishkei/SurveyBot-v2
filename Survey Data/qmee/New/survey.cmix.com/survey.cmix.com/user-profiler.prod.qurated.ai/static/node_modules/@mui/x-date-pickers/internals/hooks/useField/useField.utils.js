...
export const getLetterEditingOptions = (
  ...
  switch (sectionType) {
    case "month": {
      return getMonthsInYear(utils, utils.date(undefined, timezone)).map(
...
const isFourDigitYearFormat = (utils, timezone, format) =>
...
export const getDateFromDateSections = (utils, sections, localizedDigits) => {
  ...
  for (let i = 0; i < sections.length; i += 1) {
    ...
    if (!shouldSkip) {
      ...
      sectionValues.push(
        getSectionVisibleValue(section, "non-input", localizedDigits)
      );
    }
  }
...
export const createDateStrForV7HiddenInputFromSections = (sections) =>
  sections
    .map((section) => {
...
export const getSectionsBoundaries = (utils, localizedDigits, timezone) => {
  ...
  return {
    ...
    weekDay: ({ format, contentType }) => {
      if (contentType === "digit") {
        const daysInWeek = getDaysInWeekStr(utils, timezone, format).map(
          Number
        );
        return {
  ...
  if (process.env.NODE_ENV !== "production") {
    if (!warnedOnceInvalidSection) {
      ...
      const invalidSection = sections.find(
        (section) => !supportedSections.includes(section.type)
      );
      if (invalidSection) {
        console.warn(
  ...
  switch (section.type) {
    case "month": {
      if (section.contentType === "digit") {
        return utils.format(
          utils.setMonth(utils.date(), Number(section.value) - 1),
          "month"
        );
      }
...
export const getSectionValueNow = (section, utils) => {
  ...
  switch (section.type) {
    ...
    default:
      return section.contentType !== "letter"
        ? Number(section.value)
        : undefined;
  }
};
